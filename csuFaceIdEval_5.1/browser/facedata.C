...
/*****************************************************************************
 * this method is used to classify the covarites checking on some conditions *
 *  when saving the data. also it is called when after the load the data for *
 *      setting the same varaiables on the basis of the same conditions      *
 *****************************************************************************/ 
...
/*********************************************************************
 * Loads values form the file and sets the appropriate radio buttons *
 *      for the images saved in the file. ie it sets the correct     *
 *            covariates for the images saved in the file            *
 *********************************************************************/                   
void FaceData::LoadData(){
  ...
  cout << "Loading Data..." << endl;
    cout<<"in load file = "<< loadfile<<"\n";
    
    if(loadfile==1)
    loadpath = loadDir.path();
    else 
    loadpath = QFileDialog::getOpenFileName();	    
    
    ifstream file(loadpath);
    //cout<<"loadpath ="<<loadpath<<"\n";
    int num_images = 0;
    file >> num_images;
...
/***********************************************************************
 * Saves values of all the covariates for each of the images that have * 
 *     been activated and for which appropriate values are choosen     *
 ***********************************************************************/
